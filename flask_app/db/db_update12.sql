...
CREATE TABLE IF NOT EXISTS `db_noticias`.`reports_noticias` (
  ...
  PRIMARY KEY (`usuarios_id`, `noticias_id`),
  INDEX `fk_usuarios_has_noticias_noticias2_idx` (`noticias_id` ASC) VISIBLE,
  INDEX `fk_usuarios_has_noticias_usuarios2_idx` (`usuarios_id` ASC) VISIBLE,
  CONSTRAINT `fk_usuarios_has_noticias_usuarios2`
    FOREIGN KEY (`usuarios_id`)
    REFERENCES `db_noticias`.`usuarios` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_usuarios_has_noticias_noticias2`
...
CREATE TABLE IF NOT EXISTS `db_noticias`.`reports_comentarios` (
  ...
  PRIMARY KEY (`usuarios_id`, `comentarios_id`),
  INDEX `fk_usuarios_has_comentarios_comentarios1_idx` (`comentarios_id` ASC) VISIBLE,
  INDEX `fk_usuarios_has_comentarios_usuarios1_idx` (`usuarios_id` ASC) VISIBLE,
...
DROP TABLE IF EXISTS `db_noticias`.`reports_noticias` ;

DROP TABLE IF EXISTS `db_noticias`.`reports_comentarios` ;
